...
const MentorResetPassword = () => {
  ...
  useEffect(() => {
    const validateToken = async () => {
      try {
        await axios.get(
          `http://localhost:8000/api/mentor/reset-password/${token}`
        );
        setValidToken(true);
      } catch (error) {
    ...
    if (password === '' && confirmPassword === '') {
      ...
      try {
        const response = await axios.put(
          'http://localhost:8000/api/mentor/update-mentor-password',
          {
            token,
            password
          },
    ...
    <>
      <section className="bg-gray-100 h-screen">
        <div className="custom-container flex items-center flex-col h-full">
          <h1 className="text-center mt-20">Mentor Reset Password</h1>
